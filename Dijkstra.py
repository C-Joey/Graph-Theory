...
def dijkstra_raw(edge, from_node, to_node):
	g = defaultdict(list)
	for l, r, c in edge:
		g[l].append((c, r))
...
def dijkstra(edge, from_node, to_node):
	len_shortest_path = -1
	ret_path = []
	length, path_queue = dijkstra_raw(edge, from_node, to_node)
	if len(path_queue) > 0:
...
length, Shortest_path = dijkstra(edges, node1, node2)
